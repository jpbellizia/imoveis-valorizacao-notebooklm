# Valorizacao de Imovel

**Contexto e Objetivo:**
Este projeto tem como foco o estudo da valorização de imóveis antigos, explorando estratégias práticas e teóricas que aumentam o valor de mercado de um ativo imobiliário.

O objetivo principal é:

- Identificar fatores que impactam diretamente a valorização;
- Entender como reformas influenciam o preço final;
- Analisar métodos técnicos de avaliação imobiliária;
- Aplicar conceitos ao contexto urbano, especialmente na cidade de São Paulo.

**Curadoria de Fontes:**
As seguintes fontes foram utilizadas como base para o estudo:

- 🎥 Como aumentar o valor de venda do imóvel com reformas e melhorias (https://www.youtube.com/watch?v=dDVWXHMHTww_
- 🎥 Como SUPER VALORIZAR um imóvel com uma REFORMA? (https://www.youtube.com/watch?v=rvdxaygacg4)
- 📄 Avaliação de Imóveis e suas valorizações (PDF)
- 📄 Método para determinação de valores na avaliação imobiliária (https://www.scielo.br/j/bcg/a/mfw35HD9rZcMHRrynbCFHrH/?format=pdf&lang=pt)
- 📄 Determinantes dos preços de imóveis residenciais verticais em São Paulo (https://www.anpec.org.br/encontro/2012/inscricao/files_I/i9-3a7b8f1858120e2757d222a38932e7c6.pdf)

**Engenharia de Prompts e "Cicatrizes"**
Durante o uso do NotebookLM, foram realizados diversos testes para extrair informações relevantes.

Exemplos de Prompts Utilizados
"Quais fatores mais impactam a valorização de imóveis antigos?"
"Como reformas influenciam o preço de venda de um imóvel?"
"Quais métodos são usados para avaliação imobiliária?"
"Quais variáveis impactam o preço de imóveis em São Paulo?"

** Dificuldades Encontradas**
- Filtrar resultados específicos para a cidade de São Paulo
- Respostas muito genéricas no início
- Necessidade de refinar prompts para obter dados mais técnicos

** Estratégias de Solução (Troubleshooting)**
- Uso de prompts mais específicos (localização, tipo de imóvel)
- Cruzamento de fontes acadêmicas com conteúdo prático
- Refinamento iterativo das perguntas

** Miniguia de Estudo**
1. Resumo Estruturado

A valorização de imóveis depende de múltiplos fatores:

Fatores físicos
Estado de conservação
Qualidade da construção
Modernização (cozinha, banheiro, fachada)

Localização
Infraestrutura urbana
Segurança
Proximidade de serviços

Mercado
Oferta e demanda
Tendências econômicas
Taxas de juros

Reformas estratégicas
Pequenas reformas podem gerar alto retorno
Cozinha e banheiro são os ambientes mais valorizados
Pintura e iluminação têm alto impacto visual com baixo custo

**2. Glossário**
- Valorização imobiliária: aumento do valor de mercado de um imóvel
- Avaliação imobiliária: processo técnico para determinar o valor de um imóvel
- Regressão linear: método estatístico usado para prever valores
- Lógica fuzzy: abordagem que considera incertezas na análise
- Ativo imobiliário: bem físico com valor econômico
